...
open class FormInputListener: NSObject {
    ...
    @objc func onChange(_ sender: UITextField) {
        ...
        case is PhoneValue:
            ...
            break
        case is CardNumberValue:
            self.logicForCardNumber(text, value: val as! CardNumberValue)
        case is StringValue:
    ...
    internal func logicForCardNumber(_ text: String, value: CardNumberValue) {
        let cardNumber = value.formattedValue(text.removeSpaces())
        self.model?.setValue(cardNumber)
        (self.field as? ITextField)?.text = cardNumber
    }
